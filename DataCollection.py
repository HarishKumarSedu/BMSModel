...
class DataCollection:
    ...
    def dataCollect(self):
        try:
            ...
            for i in range(1,400002):
                ...
                if (min(soc) >= 0.05 and max(soc) <= 0.95):
                    ...
                    if(i%100 == 0):
                        ...
                        batDf = pd.DataFrame({'Index':self.index,'Sim Date Time':self.simDatetime,'vBatt0':self.voltages[0],'vBatt1':self.voltages[1],'vBatt2':self.voltages[2],'vBatt3':self.voltages[3],'vBatt4':self.voltages[4],'vBatt5':self.voltages[5],'vBatt6':self.voltages[6],'vBatt7':self.voltages[7]})
            ...
            plt.legend(["Batt0", "Batt7"], loc ="best")
            plt.show()
