...
if __name__ == '__main__':
    ...
    for i in range(10000):
        ...
        chroma.BateryEmulatorCellVoltages
        chroma.BateryEmulatorCellCurrent
        # print(time.time() - start_time)
        PacketTime.append(time.time() - start_time)
    
    max_time = max(PacketTime)   
    min_time = min(PacketTime)   
    print(f'Max Packet Time {max_time} Min Packet Time {min_time}')
    plt.plot(PacketTime)
    plt.xlabel('iterations ')
    plt.ylabel('Packet Travel Time in S')
    plt.show()
    chroma.closeBatteryEmulator()
    
    
# [4.006121, 4.006377, 4.006553, 4.006307, 4.008719, 4.009105, 4.009348, 4.009562]
# 0.019963741302490234

[3.994197, 3.994261, 3.993768, 3.994241, 3.991262, 3.991027, 3.990893, 3.990959]
